...
function NotificationTop() {
    ...
    const data = useSelector(state => state.getAllUser.receiverUser);
    ...
    return (
        <Box>
            ...
            {
                (data.length) ? data && data.map((requestedUser) => {
                    let userProfile = requestedUser.sender_user;
                    return (
                    ...
                    <Box textAlign={"center"}>
                        <h6>No Receive Request Founded.</h6>
                    </Box>
                )
            }
        </Box>
